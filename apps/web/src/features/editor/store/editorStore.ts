...
import { ensureBackgroundTrack } from '@/features/editor/lib/sceneTimeline';
...
function sanitizeTrackClips(track: Track, projectDuration: number): Clip[] {
  ...
  return sorted.map((clip) => {
    if (clip.type === 'background') {
      return {
        ...
        trackId: track.id,
        startTime: 0,
        duration: Math.max(MIN_CLIP_DURATION, projectDuration),
      };
    }

    const safeDuration = Math.max(
      MIN_CLIP_DURATION,
      Number.isFinite(clip.duration) ? clip.duration : MIN_CLIP_DURATION
    );
    const maxStart = Math.max(0, projectDuration - safeDuration);
    const boundedStart = clamp(Number.isFinite(clip.startTime) ? clip.startTime : 0, 0, maxStart);
    const nonOverlappedStart = Math.max(previousEnd, boundedStart);
    const startTime = clamp(nonOverlappedStart, 0, maxStart);
    const duration = Math.max(MIN_CLIP_DURATION, Math.min(safeDuration, projectDuration - startTime));
    ...
    return {
      ...
      startTime,
      duration,
      mediaStart: clip.mediaStart == null ? clip.mediaStart : Math.max(0, clip.mediaStart),
    };
  });
}
...
function sanitizeProject(project: Project): Project {
  const safeDuration = Math.max(
    MIN_CLIP_DURATION,
    Number.isFinite(project.duration) ? project.duration : MIN_CLIP_DURATION
  );
  return {
    ...project,
    ...
    tracks: project.tracks.map((track) => ({
      ...track,
      clips: sanitizeTrackClips(track, safeDuration),
    })),
  };
}
  ...
  switch (action.type) {
    ...
    case 'SPLIT_AT_PLAYHEAD': {
      ...
      const tracks = state.project.tracks.map((track) => {
        if (track.locked || !track.visible || track.muted) {
          return track;
        }
  ...
  dispatch: (action) =>
    set((state) => {
      ...
      if (action.type === 'REPLACE_SNAPSHOT') {
        const updatedBase = editorReducer(state as EditorState, action);
        const updated =
          updatedBase.project == null
            ? updatedBase
            : {
                ...updatedBase,
                project: sanitizeProject(ensureBackgroundTrack(updatedBase.project)),
              };
        return {
      ...
      const updatedBase = editorReducer(prevState, action);
      const updated =
        updatedBase.project == null
          ? updatedBase
          : { ...updatedBase, project: sanitizeProject(ensureBackgroundTrack(updatedBase.project)) };
