...
import type { Project, MediaFile } from '@/shared/types';
...
async function safeSet<T>(key: string, value: T): Promise<void> {
  ...
  if (desktop) {
    await desktop.storageSet(key, value);
    return;
  }
  try {
    await set(key, value);
...
async function safeGet<T>(key: string): Promise<T | undefined> {
  const desktop = getDesktopBridge();
  if (desktop) {
    const value = await desktop.storageGet(key);
...
async function readProjectIndexFromIdb(): Promise<ProjectSummary[]> {
  const desktop = getDesktopBridge();
  if (desktop) {
    const v = await desktop.storageGet(PROJECT_INDEX_KEY);
    return Array.isArray(v) ? (v as ProjectSummary[]) : [];
  }
  try {
...
async function writeProjectIndex(list: ProjectSummary[]): Promise<void> {
  ...
  const desktop = getDesktopBridge();
  if (desktop) {
    await desktop.storageSet(PROJECT_INDEX_KEY, normalized);
    return;
  }
  try {
