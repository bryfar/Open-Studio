...
import { useEditorStore, createTrack, createClip } from '@/stores/editorStore';
import { createBackgroundClip, defaultProjectBackground } from '@/lib/sceneTimeline';
import { icons } from '@/components/icons';
import { Button } from '@/components/ui/Button';
import { Slider } from '@/components/ui/Slider';
import { timelineColors, cn, formatTime, generateId } from '@/lib/utils';
import { Track, Clip, Project, MediaFile, DEFAULT_MEDIA_CROP, MediaCropRect } from '@/types';
import { interpolateCamera } from '@/lib/animation';
...
function buildSnapPoints(project: Project, excludeClipId: string | null): number[] {
...
export function Timeline({
  ...
  const handleTimelineClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      const points = buildSnapPoints(project, null);
    ...
    [dispatch, pixelsPerSecond, project, isDraggingPlayhead, resolveTimelineX, snappingEnabled]
  );
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (draggingClip && timelineRef.current && project) {
        ...
        if (clip) {
          const points = buildSnapPoints(project, draggingClip.clipId);
          const newStart = snappingEnabled
            ? snapClipStart(rawStart, clip.duration, points, thresholdSec)
            : rawStart;
          const clampedStart = Math.max(0, Math.min(project.duration - clip.duration, newStart));
      ...
      if (trimmingClip && timelineRef.current && project) {
        ...
        if (!clip) return;
        const points = buildSnapPoints(project, trimmingClip.clipId);
        ...
        if (trimmingClip.edge === 'end') {
          ...
          const nextDuration = Math.max(0.1, snappedEnd - clip.startTime);
          if (rippleEditingEnabled) {
        ...
        const safeStart = Math.max(0, proposedStart);
        ...
        const nextDuration = Math.max(0.1, trimmingClip.originalDuration - deltaStart);
  ...
  const toggleRippleEditing = useCallback(() => {
    setRippleEditingEnabled((prev) => !prev);
  }, []);
  ...
  const contentMinHeight = 36 + project.tracks.length * 56;
